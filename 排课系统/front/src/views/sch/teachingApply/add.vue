    ...
    <Card>
        <div slot="title">
            <div class="edit-head">
                ...
                <div class="head-name">添加排课申请</div>
                <span></span>
        ...
        <Form ref="form" :model="form" :label-width="100" :rules="formValidate" label-position="left">
            <FormItem label="排课" prop="scheduleId">
                <Select v-model="form.scheduleId" clearable style="width:570px">
            ...
            <FormItem label="地点">
                <Input style="width: 570px;" placeholder="请填写地点" v-model="form.place"/>
            </FormItem>
            <Form-item class="br">
...
export default {
    ...
    methods: {
        init() {
            ...
            this.getTeachingScheduleListFx();
        },
        getCurriculumListFx() {
            var that = this;
            that.curriculumList = [];
        ...
        getTeachingScheduleListFx() {
            var that = this;
            that.scheduleList = [];
            getTeachingScheduleList().then(res => {
                if (res.success) {
                    that.scheduleList = res.result;
                }
            })
        },
        handleReset() {
        ...
        handleSubmit() {
            this.$refs.form.validate(valid => {
                if (valid) {
                    addTeachingApply(this.form).then(res => {
